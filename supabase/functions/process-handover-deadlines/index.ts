...
async function buildAutoAcceptPdf(p: Record<string, unknown>): Promise<string> {
  ...
  let y = 800;
  const marginX = 55;
  const width = 595 - 2 * marginX;

  page.drawText("Übernahmeprotokoll", { x: marginX, y, size: 22, font: bold, color: rgb(0.1, 0.1, 0.1) });
  if (logoImg) {
    const logoW = 70;
    const scale = logoW / logoImg.width;
    const logoH = logoImg.height * scale;
    page.drawImage(logoImg, { x: 595 - marginX - logoW, y: y - 10, width: logoW, height: logoH });
  }
  y -= 12;
  page.drawLine({ start: { x: marginX, y }, end: { x: 595 - marginX, y }, thickness: 1.5, color: rgb(0.1, 0.1, 0.1) });
  y -= 25;

  page.drawText("EINZUG · AUTOMATISCH AKZEPTIERT", { x: marginX, y, size: 9, font: bold, color: rgb(0.5, 0.5, 0.5) });
  y -= 25;

  const H = (label: string) => {
    page.drawText(label, { x: marginX, y, size: 10, font: bold, color: rgb(0.1, 0.1, 0.1) });
    y -= 15;
  };
  const R = (lbl: string, val: string) => {
    page.drawText(lbl, { x: marginX, y, size: 10, font: bold, color: rgb(0.3, 0.3, 0.3) });
    page.drawText(val, { x: marginX + 120, y, size: 10, font, color: rgb(0, 0, 0) });
    y -= 14;
  };

  H("MIETER*IN");
  R("Vorname:", (p.mieter_vorname as string) || "-");
  R("Nachname:", (p.mieter_nachname as string) || "-");
  if (p.mieter_geburtsdatum) R("Geburtsdatum:", formatDateDE(p.mieter_geburtsdatum as string));
  if (p.mieter_nationalitaet) R("Nationalität:", p.mieter_nationalitaet as string);
  if (p.mieter_email) R("E-Mail:", p.mieter_email as string);
  if (p.mieter_telefon) R("Telefon:", p.mieter_telefon as string);
  y -= 8;

  H("MIETOBJEKT");
  if (p.apt_adresse) {
    const adr = `${p.apt_adresse}${p.apt_plz || p.apt_ort ? ", " : ""}${p.apt_plz || ""} ${p.apt_ort || ""}`.trim();
    R("Wohnung:", adr);
  }
  if (p.room_label) R("Zimmer:", p.room_label as string);
  if (p.einzug_datum) R("Einzug am:", formatDateDE(p.einzug_datum as string));
  if (p.deadline) R("Frist bis:", formatDateDE(p.deadline as string));
  y -= 8;

  if (p.schluessel_info) {
    H("SCHLÜSSELÜBERGABE");
    const lines = (p.schluessel_info as string).split("\n");
  ...
  page.drawRectangle({ x: marginX, y: y - 55, width, height: 55, color: rgb(0.91, 0.94, 0.87), borderColor: rgb(0.77, 0.84, 0.69), borderWidth: 1 });
  y -= 18;
  page.drawText("STATUS", { x: marginX + 10, y, size: 8, font: bold, color: rgb(0.23, 0.32, 0.19) });
  y -= 13;
  page.drawText("Frist abgelaufen ohne Meldung von Mängeln.", { x: marginX + 10, y, size: 10, font: bold, color: rgb(0, 0, 0) });
  y -= 13;
  page.drawText("Die Wohnung gilt als in einwandfreiem Zustand übernommen.", { x: marginX + 10, y, size: 9.5, font, color: rgb(0.2, 0.2, 0.2) });
  y -= 30;

  H("ERKLÄRUNG DER VERTRAGSPARTEIEN");
  const klauseln = [
    "1. Da innert der 14-tägigen Frist keine sichtbaren Mängel gemeldet wurden,",
    "    gilt die Wohnung als in einwandfreiem Zustand übernommen.",
    "2. Verdeckte Mängel, die bei Übernahme trotz sorgfältiger Prüfung nicht",
    "    erkennbar waren, können weiterhin innert 10 Tagen ab Entdeckung",
    "    an info@dthomes.ch gemeldet werden (Art. 256 OR).",
    "3. Die Mietpartei verpflichtet sich, das Mietobjekt sorgfältig zu nutzen",
    "    und in vergleichbarem Zustand zurückzugeben.",
    "4. Bei Streitigkeiten gilt schweizerisches Recht; Gerichtsstand ist Chur.",
  ];
  for (const line of klauseln) {
    page.drawText(line, { x: marginX, y, size: 9.5, font, color: rgb(0.2, 0.2, 0.2) });
    y -= 12.5;
  }

  y = 55;
  page.drawLine({ start: { x: marginX, y: y + 15 }, end: { x: 595 - marginX, y: y + 15 }, thickness: 0.5, color: rgb(0.7, 0.7, 0.7) });
  page.drawText(`Auto-akzeptiert am ${formatDateDE(new Date().toISOString())}`, { x: marginX, y, size: 8.5, font, color: rgb(0.4, 0.4, 0.4) });
  page.drawText("D&T Partners GmbH · dthomes.ch", { x: marginX, y: y - 12, size: 8.5, font, color: rgb(0.4, 0.4, 0.4) });
    ...
    for (const p of protocols) {
      try {
        ...
        if (daysLeft <= 0 && !p.auto_accept_email_sent_at) {
          ...
          try { pdfBase64 = await buildAutoAcceptPdf(p); } catch (pdfErr) { console.error(`PDF generation failed for ${p.id}:`, pdfErr); }
